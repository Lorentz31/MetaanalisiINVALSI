...
#
df <- data.frame(read.csv(paste0(DataFolder, "DataMatMods.csv")))
...
#
png("GraphProvinces_Mat_logvr.png", width = 1300, height = 1500)
...
#
png("GraphRegions__Mat_logvr.png", width = 1300, height = 1500)
...
#
levels(factor(RandEffP$Cod_provincia_ISTAT))==levels(factor(shp$COD_PROV))

#
df <- merge(shp, RandEffP, by.x = "COD_PROV", by.y = "Cod_provincia_ISTAT", all=T)

#
png("GraphProvinces__Mat_RandEffP.png", width = 1300, height = 1500)
...
#
png("GraphRegions__Mat_RandEffR.png", width = 1300, height = 1500)
